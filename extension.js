...
function js_beautify2(js_source_text, options=undefined) {
...
function Beautifier(js_source_text, options=undefined) {
    var output;
    ...
    this.beautify = function() {
        ...
        Tokenizer = new tokenizer(js_source_text,opt);
    ...
    function print_newline(force_newline=null, preserve_statement_flags=null) {
    ...
    function print_token(printable_token=undefined) {
        if (output.raw) {
    ...
    function get_token(offset=undefined) {
    ...
    function handle_start_block() {
        ...
        if (opt.brace_style === "expand" || (opt.brace_style === "none" && current_token.wanted_newline)) {
            ...
            if (is_array(previous_flags.mode) && (last_type === 'TK_START_EXPR' || last_type === 'TK_COMMA')) {
                ...
                if (opt.brace_style === 'collapse-preserve-inline' && (last_type === 'TK_COMMA' || (last_type === 'TK_START_EXPR' && flags.inline_frame))) {
                    allow_wrap_or_preserved_newline();
    ...
    function handle_word() {
        ...
        if (last_type === 'TK_END_BLOCK') {
            if (!(current_token.type === 'TK_RESERVED' && in_array(current_token.text, Tokenizer.trailing_reserved_words))) {
                prefix = 'NEWLINE';
            } else {
...
var Token = function(type, text, newlines=undefined, whitespace_before=undefined, parent=undefined) {
    this.type = type;
    this.text = text;
...
function tokenizer(input, opts) {
    ...
    this.line_starters = 'if switch case default for while break function import export private params'.split(' ');
    // Reserved words that follow a block and should not be put on a new line after the closing brace.
    this.trailing_reserved_words = 'else from do then foreach'.split(' ');
    var reserved_words = this.line_starters.concat('in else typeof as'.split(' ')).concat(this.trailing_reserved_words);
    ...
    this.tokenize = function() {
        ...
        while (!(last && last.type === 'TK_EOF')) {
            ...
            next = new Token(token_values[1],token_values[0],n_newlines,whitespace_before_token,null);
    ...
    function tokenize_next() {
        ...
        if (c === "'" || c === '"') {
            ...
            resulting_string = c;
            var parse_string = function(delimiter, allow_unescaped_newlines=undefined) {
                var current_char;
                while (parser_pos < input_length) {
                    current_char = input.charAt(parser_pos);
                    if (!(esc || current_char !== delimiter)) {
                        // If it is a double delimiter it escapes to a single one
                        if (current_char === delimiter && parser_pos+1 < input_length && input.charAt(parser_pos+1) === delimiter) {
                            resulting_string += delimiter;
                            parser_pos += 1;
                        } else {
                            break;
                        }
        ...
        if (c === '#') {
            ...
            if (parser_pos < input_length && digit.test(input.charAt(parser_pos))) {
                do {
                    ...
                    sharp += c;
                    parser_pos += 1;
                } while (parser_pos < input_length && c !== '#' && c !== '=');if (c === '#') {} else if (input.charAt(parser_pos) === '[' && input.charAt(parser_pos + 1) === ']') {
                    sharp += '[]';
                    parser_pos += 2;
...
function activate(context) {
    vscode.languages.registerDocumentFormattingEditProvider('sqf', {
        /**
         * @param {vscode.TextDocument} document
         * @returns {vscode.TextEdit[]}
         */
        provideDocumentFormattingEdits(document) {
            var beatifiedText = js_beautify2(document.getText());
            ...
            return [vscode.TextEdit.replace(textRange, beatifiedText)];
        }
    });
    vscode.languages.registerDocumentRangeFormattingEditProvider('sqf', {
        /**
         * @param {vscode.TextDocument} document
        ...
        provideDocumentRangeFormattingEdits(document, range) {
            var beatifiedText = js_beautify2(document.getText(range));
            var firstLine = document.lineAt(0);
            var lastLine = document.lineAt(document.lineCount - 1);
            return [vscode.TextEdit.replace(range, beatifiedText)];
        }
    });
}
...
//     console.log('Congratulations, your extension "sqf-formatter" is now active!');
