...
var object_wrapper:any = Object;

if (!object_wrapper.values) {
    object_wrapper.values = function(o) {
        if (o !== Object(o)) {
            throw new TypeError('object_wrapper.values called on a non-object');
        }
        var k = [], p;
        for (p in o) {
...
function sanitizeOperatorPosition(opPosition) {
    ...
    var validPositionValues = object_wrapper.values(OPERATOR_POSITION);
...
function tokenizer(input, opts) {
    ...
    this.line_starters = 'continue,try,throw,return,var,let,const,if,switch,case,default,for,while,break,function,import,export,private,params'.split(',');
    var reserved_words = this.line_starters.concat(['do', 'in', 'else', 'get', 'set', 'new', 'catch', 'finally', 'typeof', 'yield', 'async', 'await', 'from', 'as', 'then', 'forEach']);
    ...
    function tokenize_next() {
        ...
        if (c === '/') {
            ...
            if (input.charAt(parser_pos) === '*') {
                ...
                var directives:any = get_directives(comment);
